...
const Admin: React.FC = () => {
  ...
  const handleForgotPassword = (e: React.MouseEvent) => {
    ...
    if (window.confirm(`비밀번호 재설정을 위해 관리자 메일(${email})로 이메일을 보내시겠습니까?`)) {
      window.location.href = `mailto:${email}?subject=Admin Password Inquiry&body=Requesting password assistance for BBEOGGUGI admin panel.`;
    }
  };
  ...
  const deleteInquiry = (e: React.MouseEvent, id: number) => {
    e.stopPropagation();
    if (window.confirm('이 문의 내역을 정말로 삭제하시겠습니까? (삭제 시 복구 불가)')) {
      const updated = inquiries.filter(inq => inq.id !== id);
      setInquiries(updated);
      saveData('bbeoggugi_inquiries', updated, true);
      alert('문의가 삭제되었습니다.');
    }
  };
  ...
  const deleteProject = (e: React.MouseEvent, id: string) => {
    e.stopPropagation();
    if (window.confirm('이 프로젝트를 정말로 삭제하시겠습니까? (삭제 시 복구 불가)')) {
      const updated = projects.filter(p => p.id !== id);
      setProjects(updated);
      saveData('bbeoggugi_projects', updated, true);
      alert('프로젝트가 삭제되었습니다.');
    }
  };
        ...
        <div className="w-full max-w-md space-y-12 text-center">
          <form onSubmit={handleLogin} className={`space-y-12 transition-transform ${loginError ? 'translate-x-1' : ''}`}>
            ...
            <div className="space-y-6">
              ...
              <button type="submit" className="w-full bg-black text-white px-12 py-5 rounded-full uppercase tracking-[0.4em] text-[10px] font-black shadow-xl">Login</button>
            </div>
          </form>
          ...
          {activeTab === 'projects' && (
            <div className="space-y-8">
              ...
              <div className="grid grid-cols-1 gap-4">
                {projects.map((p, idx) => (
                  <div key={p.id} className="border bg-white rounded-sm overflow-hidden transition-all">
                    <div className="p-5 flex justify-between items-center bg-gray-50/50">
                       ...
                       <div className="flex gap-4">
                          <button onClick={() => setEditingProjectId(editingProjectId === p.id ? null : p.id)} className="text-[10px] font-bold border px-6 py-2 rounded-full hover:bg-black hover:text-white transition-all">EDIT</button>
                          <button onClick={(e) => deleteProject(e, p.id)} className="text-[10px] font-bold border border-red-50 text-red-400 px-4 py-2 rounded-full hover:bg-red-500 hover:text-white transition-all">DEL</button>
                       </div>
                    </div>
                      ...
                      <div className="p-8 space-y-12 animate-in slide-in-from-top-4 duration-300">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                           <div className="space-y-6">
                              ...
                              <input className="w-full border-b p-2 kor-bold" value={p.title} onChange={e => { const u = [...projects]; u[idx].title = e.target.value; setProjects(u); }} placeholder="Project Title (KR)" />
                              <input className="w-full border-b p-2 eng-text text-sm" value={p.titleEn} onChange={e => { const u = [...projects]; u[idx].titleEn = e.target.value; setProjects(u); }} placeholder="PROJECT TITLE (EN)" />
                              <textarea className="w-full border p-3 text-xs leading-relaxed" rows={4} value={p.descriptionKr} onChange={e => { const u = [...projects]; u[idx].descriptionKr = e.target.value; setProjects(u); }} placeholder="Description (KR)" />
                           </div>
                           <div className="space-y-6">
                              ...
                              <div className="grid grid-cols-2 gap-4">
                                 {Object.keys(p.info).map((key) => (
                                   <div key={key}>
                                     ...
                                     <input className="w-full border-b py-1 text-xs" value={(p.info as any)[key]} onChange={e => { const u = [...projects]; (u[idx].info as any)[key] = e.target.value; setProjects(u); }} />
                                   </div>
                                 ))}
                              </div>
                           </div>
                        ...
                        <div className="space-y-6">
                           <label className="text-[10px] uppercase font-black text-gray-400 block border-b pb-1">Gallery Images</label>
                           <div className="grid grid-cols-2 md:grid-cols-5 lg:grid-cols-6 gap-4">
                              {p.images.map((img, iidx) => (
                                <div key={iidx} className="relative aspect-[3/4] border overflow-hidden group">
                                   ...
                                   <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex flex-col justify-between p-2">
                                      <div className="flex justify-between">
                                         ...
                                         <button onClick={() => { const u = [...projects]; u[idx].images = u[idx].images.filter((_, i) => i !== iidx); setProjects(u); }} className="bg-red-500 text-white p-1 text-[8px]">DEL</button>
                                      ...
                                      <button onClick={() => { const u = [...projects]; u[idx].mainImage = img; setProjects(u); }} className="text-[8px] font-black text-white bg-black/40 py-1 uppercase tracking-tighter">Set as Main</button>
                                   </div>
                                </div>
                              ))}
                              <label className="aspect-[3/4] border-2 border-dashed flex flex-col items-center justify-center cursor-pointer text-gray-300 hover:text-black transition-colors">
                        ...
                        <button onClick={() => saveData('bbeoggugi_projects', projects)} className="w-full bg-black text-white py-4 font-black uppercase tracking-[0.4em]">Save Changes</button>
                      </div>
                    )}
                  </div>
                ))}
            ...
            <div className="space-y-8 animate-in fade-in">
               ...
               <div className="overflow-x-auto">
                 <table className="w-full text-left">
                    <thead className="border-b text-[10px] uppercase text-gray-400 font-black">
                       <tr>
                       ...
                       {inquiries.length > 0 ? inquiries.map(inq => (
                         <React.Fragment key={inq.id}>
                           <tr className={`cursor-pointer transition-colors ${expandedInquiryId === inq.id ? 'bg-gray-50' : 'hover:bg-gray-50'}`} onClick={() => setExpandedInquiryId(expandedInquiryId === inq.id ? null : inq.id)}>
                              <td className="py-6 px-4 text-xs italic text-gray-400">{new Date(inq.date).toLocaleDateString()}</td>
                              <td className="py-6 px-4"><span className="text-[10px] font-black uppercase border px-2 py-1">{inq.type}</span></td>
                              <td className="py-6 px-4 kor-bold text-sm">{inq.name}</td>
                              <td className="py-6 px-4"><button onClick={(e) => deleteInquiry(e, inq.id)} className="text-red-400 text-[10px] font-black uppercase hover:underline">Delete</button></td>
                           </tr>
                                ...
                                <td colSpan={4} className="p-8">
                                   <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                                      <div className="space-y-4">
                                         ...
                                         <div className="bg-white p-6 border rounded-sm text-sm leading-relaxed whitespace-pre-wrap">{inq.message || inq.content || 'No text provided.'}</div>
                                      </div>
                         ...
                         <tr><td colSpan={4} className="py-20 text-center text-gray-300 text-xs">No records found.</td></tr>
                       )}
                    </tbody>
                 </table>
               </div>
            ...
            <div className="space-y-12 animate-in fade-in">
               <div className="flex gap-8 border-b mb-10 overflow-x-auto scrollbar-hide">
                 {(['about', 'space', 'contact', 'journal', 'career'] as const).map(sub => (
                   <button key={sub} onClick={() => setActiveSettingsSubTab(sub)} className={`pb-4 text-[10px] font-black uppercase tracking-[0.3em] transition-all ${activeSettingsSubTab === sub ? 'border-b-2 border-black opacity-100' : 'opacity-20 hover:opacity-100'}`}>
                     {sub}
                   </button>
                 ))}
               </div>
               ...
               <div className="max-w-4xl space-y-12">
                    ...
                    <div className="space-y-12 animate-in slide-in-from-top-4">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                          <div className="space-y-4">
                             ...
                             <textarea className="w-full border-b p-2 kor-bold text-xl h-20 outline-none" value={careerContent.mainTitle} onChange={e => setCareerContent({...careerContent, mainTitle: e.target.value})} placeholder="Title" />
                             <textarea className="w-full border p-3 text-xs h-32 outline-none" value={careerContent.descKr} onChange={e => setCareerContent({...careerContent, descKr: e.target.value})} placeholder="Description" />
                          </div>
                          <div className="space-y-4">
                             ...
                             <div className="aspect-[3/4] bg-gray-50 border overflow-hidden relative group">
                                ...
                                <label className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 flex items-center justify-center cursor-pointer text-white text-[9px] font-black">UPLOAD<input type="file" className="hidden" onChange={async (e: React.ChangeEvent<HTMLInputElement>) => { if(e.target.files?.[0]) { const b64 = await handleFileToBase64(e.target.files[0]); setCareerContent({...careerContent, mainImage: b64}); } }} /></label>
                             </div>
                          </div>
                       </div>
                       ...
                       <div className="space-y-6">
                          ...
                          <div className="space-y-4">
                            {careerContent.coreValues?.map((v:any, i:number) => (
                              <div key={i} className="flex gap-4 border p-4 items-start relative group bg-white">
                                 <input className="text-sm kor-bold w-1/3 outline-none border-b" value={v.kr} onChange={e => { const u = [...careerContent.coreValues]; u[i].kr = e.target.value; setCareerContent({...careerContent, coreValues: u}) }} />
                                 <textarea className="text-xs flex-grow outline-none border p-2 h-16" value={v.desc} onChange={e => { const u = [...careerContent.coreValues]; u[i].desc = e.target.value; setCareerContent({...careerContent, coreValues: u}) }} />
                                 <button onClick={() => { const u = careerContent.coreValues.filter((_:any,idx:number)=>idx!==i); setCareerContent({...careerContent, coreValues: u}) }} className="text-red-400 opacity-0 group-hover:opacity-100 text-[9px] font-black uppercase">Del</button>
                              </div>
                            ))}
                       ...
                       <div className="space-y-6">
                          ...
                          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {careerContent.benefits?.map((b:any, i:number) => (
                              <div key={i} className="flex gap-4 border p-4 items-start relative group bg-white">
                                 <input className="text-sm kor-bold w-1/3 outline-none border-b" value={b.kr} onChange={e => { const u = [...careerContent.benefits]; u[i].kr = e.target.value; setCareerContent({...careerContent, benefits: u}) }} />
                                 <input className="text-[10px] flex-grow outline-none border-b" value={b.desc} onChange={e => { const u = [...careerContent.benefits]; u[i].desc = e.target.value; setCareerContent({...careerContent, benefits: u}) }} />
                                 <button onClick={() => { const u = careerContent.benefits.filter((_:any,idx:number)=>idx!==i); setCareerContent({...careerContent, benefits: u}) }} className="text-red-400 opacity-0 group-hover:opacity-100 text-[9px] font-black uppercase">Del</button>
                              </div>
                            ))}
                       ...
                       <div className="space-y-6">
                          ...
                          <div className="space-y-4">
                            {careerContent.positions?.map((p:any, i:number) => (
                              <div key={i} className="border p-6 space-y-4 relative group bg-white">
                                 <input className="w-full kor-bold text-lg outline-none border-b" value={p.title} onChange={e => { const u = [...careerContent.positions]; u[i].title = e.target.value; setCareerContent({...careerContent, positions: u}) }} placeholder="Job Title" />
                                 <input className="w-full text-xs text-gray-400 outline-none border-b" value={p.subTitle} onChange={e => { const u = [...careerContent.positions]; u[i].subTitle = e.target.value; setCareerContent({...careerContent, positions: u}) }} placeholder="Sub Title (e.g. 경력 3년 이상)" />
                                 <input className="w-full text-xs text-gray-300 outline-none border-b" value={p.type} onChange={e => { const u = [...careerContent.positions]; u[i].type = e.target.value; setCareerContent({...careerContent, positions: u}) }} placeholder="Job Type" />
                                 <textarea className="w-full text-xs border p-2 h-20 outline-none" value={p.desc} onChange={e => { const u = [...careerContent.positions]; u[i].desc = e.target.value; setCareerContent({...careerContent, positions: u}) }} placeholder="Job Details" />
                                 <button onClick={() => { const u = careerContent.positions.filter((_:any,idx:number)=>idx!==i); setCareerContent({...careerContent, positions: u}) }} className="absolute top-4 right-4 text-red-400 opacity-0 group-hover:opacity-100 text-[9px] font-black">DELETE</button>
                              </div>
                            ))}
                            <button onClick={() => setCareerContent({...careerContent, positions: [...(careerContent.positions || []), {id: Date.now().toString(), title: 'Position Title', subTitle: 'Sub Title', type: 'Full-time', desc: 'Details...'}]})} className="text-[9px] font-black border border-black p-4 w-full">+ Add Position</button>
                          </div>
                       </div>
                       <button onClick={() => saveData('bbeoggugi_career_content', careerContent)} className="w-full bg-black text-white py-5 rounded-full font-black uppercase tracking-[0.3em] shadow-xl">Apply All Career Changes</button>
                    </div>
                  )}
                    ...
                    <div className="space-y-10 animate-in fade-in">
                       <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                          <div className="space-y-4">
                             ...
                             <textarea className="w-full border-b p-2 kor-bold text-2xl h-24 outline-none" value={aboutContent.mainTitle} onChange={e => setAboutContent({...aboutContent, mainTitle: e.target.value})} />
                             <textarea className="w-full border p-3 text-xs h-32 outline-none" value={aboutContent.descKr1} onChange={e => setAboutContent({...aboutContent, descKr1: e.target.value})} />
                          </div>
                          <div className="space-y-4">
                             <label className="text-[9px] font-black uppercase text-gray-400 tracking-widest border-b pb-1 block">Bio Photo</label>
                             <div className="aspect-[4/3] border overflow-hidden relative group">
                       ...
                       <button onClick={() => saveData('bbeoggugi_about', aboutContent)} className="w-full bg-black text-white py-4 font-black uppercase tracking-[0.4em]">Save About Page</button>
                    </div>
                  )}
                    ...
                    <div className="space-y-8 animate-in fade-in">
                       ...
                       <div className="space-y-4">
                          <div className="space-y-1">
                            ...
                            <input className="w-full border-b p-2 kor-bold text-xl" value={spaceHeader.title} onChange={e => setSpaceHeader({...spaceHeader, title: e.target.value})} placeholder="Title" />
                          </div>
                          <div className="space-y-1">
                            <label className="text-[8px] uppercase font-bold text-gray-300">Archive Subtitle</label>
                            <input className="w-full border-b p-2 eng-text text-sm" value={spaceHeader.subtitle} onChange={e => setSpaceHeader({...spaceHeader, subtitle: e.target.value})} placeholder="Subtitle" />
                          </div>
                       </div>
                       <button onClick={() => saveData('bbeoggugi_space_header', spaceHeader)} className="w-full bg-black text-white py-4 font-black uppercase tracking-[0.4em]">Save Space Header</button>
                    </div>
                  )}
                  ...
                  {activeSettingsSubTab === 'contact' && (
                    <div className="space-y-8 animate-in fade-in">
                       ...
                       <div className="space-y-6">
                          <div className="space-y-4">
                             <input className="w-full border-b p-2 kor-bold text-xl" value={contactSettings.title} onChange={e => setContactSettings({...contactSettings, title: e.target.value})} placeholder="Form Title" />
                             <input className="w-full border-b p-2 eng-text text-sm" value={contactSettings.subtitle} onChange={e => setContactSettings({...contactSettings, subtitle: e.target.value})} placeholder="Form Subtitle" />
                          </div>
                       ...
                       <button onClick={() => saveData('bbeoggugi_contact_settings', contactSettings)} className="w-full bg-black text-white py-4 font-black uppercase tracking-[0.4em]">Save Contact Fields</button>
                    </div>
                  )}
                    ...
                    <div className="space-y-8 animate-in fade-in">
                       ...
                       <div className="space-y-6">
                          {journalChannels.map((ch, i) => (
                            <div key={ch.id} className="p-6 border bg-white space-y-4">
                               <div className="flex justify-between items-center">
                                  <span className="kor-bold text-sm">{ch.name}</span>
                                  <span className="eng-text text-[9px] text-gray-300 uppercase">{ch.id}</span>
                               </div>
                               <div className="space-y-3">
                                  <div className="space-y-1">
                                     <label className="text-[8px] uppercase font-bold text-gray-300">Link URL</label>
                                     <input className="w-full border-b py-2 text-xs" value={ch.url} onChange={e => {
                                  ...
                                  <div className="space-y-1">
                                     ...
                                     <div className="flex gap-4 items-center">
                                        <input className="flex-grow border-b py-2 text-xs" value={ch.thumbnail} onChange={e => {
                       ...
                       <button onClick={() => saveData('bbeoggugi_journal_channels', journalChannels)} className="w-full bg-black text-white py-4 font-black uppercase tracking-[0.4em]">Save Journal Links</button>
                    </div>
                  )}
               </div>
            </div>
            ...
            <div className="space-y-12 animate-in fade-in">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 gap-20">
                  ...
                  <div className="space-y-8">
                     ...
                     <div className="flex gap-4">
                        <button onClick={() => setDesignSettings({...designSettings, mainFont: 'serif'})} className={`flex-grow py-4 text-[10px] font-black border transition-all ${designSettings.mainFont === 'serif' ? 'bg-black text-white' : 'bg-white'}`}>SERIF (명조)</button>
                        <button onClick={() => setDesignSettings({...designSettings, mainFont: 'sans'})} className={`flex-grow py-4 text-[10px] font-black border transition-all ${designSettings.mainFont === 'sans' ? 'bg-black text-white' : 'bg-white'}`}>SANS (고딕)</button>
                     </div>
                  </div>
               </div>
               <button onClick={() => saveData('admin_design_settings', designSettings)} className="w-full bg-black text-white py-5 font-black uppercase tracking-[0.4em] shadow-xl">Apply Brand Style</button>
            </div>
          )}
        </div>
      </div>
